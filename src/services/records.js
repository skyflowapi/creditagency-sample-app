...
export const bulkGetRecords = (tableName, options, accessToken, callback) => {
  axios
    .get(`https://sb.area51.vault.skyflowapis.com/v1/vaults/${VAULT_PARAMS.VAULT_ID}/` + tableName, {
...
export const getRecord = (
  ...
  axios
    .get(
      `https://sb.area51.vault.skyflowapis.com/v1/vaults/${VAULT_PARAMS.VAULT_ID}/` + tableName + `/${recordId}`,
      {
        headers: { Authorization: "Bearer " + accessToken },
        params: {
          redaction: options.redaction,
...
export const insertRecord = (tableName, records, accessToken, callback) => {
  axios
    .post(`https://sb.area51.vault.skyflowapis.com/v1/vaults/${VAULT_PARAMS.VAULT_ID}/` + tableName, records, {
      headers: { Authorization: "Bearer " + accessToken },
    })
    .then((res) => {
      if (callback) {
...
export const searchQuery = (query, accessToken, callback) => {
  axios
    .post(`https://sb.area51.vault.skyflowapis.com/v1/vaults/${VAULT_PARAMS.VAULT_ID}/query`, query, {
      headers: { Authorization: "Bearer " + accessToken },
    })
    .then((res) => {
      if (callback) {
